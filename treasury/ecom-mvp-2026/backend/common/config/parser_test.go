...
func TestUseNonePrefixEnvAsDefaultValue(t *testing.T) {
	...
	t.Run("parsing unsupported field returns error", func(t *testing.T) {
		_, err := ParseEnv[int](env.Options{}) // passing an int instead of struct directly fails env.Parse
		assert.NotNil(t, err)
	})
}

...
func TestRefIDMiddleware(t *testing.T) {
	...
	t.Run("empty header key fallback", func(t *testing.T) {
		handler := func(c *gin.Context) {
			if v, ok := c.Request.Context().Value(refIDKey).(string); ok {
				c.String(200, v)
				return
		...
		c, e := gin.CreateTestContext(w)
		req := httptest.NewRequest(http.MethodPost, "/api", nil)
		req.Header.Add(DefaultRefIDHeaderKey, "1234")
		c.Request = req

		e.Use(RefIDMiddleware(""))
		e.POST("/api", handler)
		e.HandleContext(c)
		...
		if w.Body.String() != "1234" {
			t.Errorf("expected 1234 but got %s", w.Body.String())
		}
	})
}
...
func TestForwardRefOption(t *testing.T) {
	...
	if givenRedID != req.Header.Get(DefaultRefIDHeaderKey) {
		t.Errorf("%s is expected but got %s\n", givenRedID, req.Header.Get(DefaultRefIDHeaderKey))
	}
}

...
import (
	...
	crypt_mocks "gitlab.com/b2c-e-commerce-platform/platform/backend/common/crypt/mocks"
	hash_mocks "gitlab.com/b2c-e-commerce-platform/platform/backend/common/hash/mocks"
	...
	access_mocks "gitlab.com/b2c-e-commerce-platform/platform/backend/go-template/app/auth/access/mocks"
...
func TestResolveIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			prepare: func(m mockArgs, args args) {
				...
				m.memberStorage.
					EXPECT().
					GetMemberByEmail(args.ctx, "hashed_email").
					Return(access.Member{}, access.ErrMemberNotFound)
			},
			args: args{
				req: auth.ResolveIdentityRequest{
					GoogleAccessToken: "valid_google_access_token",
		...
		{
			name: "fail, case memberInfo GetID error",
			prepare: func(m mockArgs, args args) {
				m.googleClient.
					EXPECT().
					ValidateAccessToken(args.ctx, args.req.GoogleAccessToken).
		...
		{
			name: "fail, case organizationMember GetOrganizationID error",
			prepare: func(m mockArgs, args args) {
				m.googleClient.
					EXPECT().
					ValidateAccessToken(args.ctx, args.req.GoogleAccessToken).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m := mockArgs{
				...
				cipher:              crypt_mocks.NewCipherMock(t),
			}

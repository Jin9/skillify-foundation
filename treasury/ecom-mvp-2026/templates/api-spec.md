# API Spec Template (per-API MD)

**Parent:** [`../templates.md`](../templates.md)
**Owner role:** [Tech-Designer](../roles.md) · **`template_version`:** 0.1.0

One MD per API endpoint, grouped by `<service>-<aggregate>` folder. The user explicitly asked: "spec of API (that we have to create template and skill for let's LLM learn this), separate by file per API and group it with folder naming like `<service_name>-<aggregate_name>`".

**File location:** `<workflow_root>/design/spec/<service>-<aggregate>/<API_NAME>.md`

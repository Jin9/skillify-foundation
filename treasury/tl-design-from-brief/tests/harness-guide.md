...
3. The four invariant docs under `assertions/` hold on that output.
...
- `assertions/event-split-discipline.md` — every event is in exactly one of
  `event_catalog.domain_events` / `process_events`; command/event naming
  rules; no orphan event.

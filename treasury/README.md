...
| [`delegating-to-cli-models`](./delegating-to-cli-models/) | Drive external model CLIs as advisory sub-agents from one orchestrating agent: codex (GPT, gpt-5.6-sol) and agy (Antigravity/Gemini), plus optional headless self-delegation. Dispatch headlessly under a watchdog, ground-truth the model that actually answered, then adjudic... | skillify | examples · references · scripts · templates |

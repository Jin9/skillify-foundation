...
`run_dir` is **orchestration state** with `source: workflow.runtime.run_dir` (not a workflow input nor a stage output). It is authoritative for two purposes: (1) locating the optional `metrics.json` sidecar, and (2) writing the output `00-run_report.md`. Rationale and the `workflow.runtime.<x>` convention: references/report-anatomy.md.
...
4. **Compose `00-run_report.md`** with 9 sections in this exact order — H1, the metrics-incomplete banner (below, verbatim), header line, `## Per-stage stats`, `## Pipeline integrity`, `## File inventory`, `## Distillation`, `## Top risks`, `## Notes`. Full section spec (per-section content and ordering): references/report-anatomy.md.

   **Metrics-incomplete banner** — render this block ONLY if `metrics_incomplete` is true (from step 2); otherwise emit nothing for this slot:
   ```
...
The "Key output" column renders a stage-specific content-metric string (e.g. plan-research → `<N> sub-questions, <M> dimensions`). The Total row sums tokens, tools, and wall across the 5 prior stages only (report-run's own metrics are not double-counted). Full per-stage cell-value table: references/report-anatomy.md.
...
Avoid: rewriting the final report's prose, adding new risks/commentary, failing when metrics.json is missing, silently rendering `—` cells without the banner, recomputing token usage from scratch, omitting the file inventory, and embedding the full final report. Full matrix (why-bad + what-to-do-instead for each): references/report-anatomy.md.

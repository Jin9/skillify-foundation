...
- **`literature/`** — 55 curated source documents (~758K words) across the Anthropic, OpenAI, Copilot, open-standard, platform-rule, and academic agent ecosystems. The evidence base. Read-only research corpus; do not edit source files.
...
- **`treasury/`** — 34 production skills built with `skillify`, grouped into 7 functional categories. `treasury/README.md` is the **authoritative catalog** (the root `README.md` count can lag — trust `treasury/README.md`).
